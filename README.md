...
| **Nginx 통합** | http://192.168.0.92:8106 (내부망 전용) |
...
진입점은 **내부망에서만** 접근할 수 있습니다 (nginx IP 허용목록, 그 밖에서는 403).
호출자 위치에 따라 주소가 다릅니다.

| 호출자 | 주소 |
|--------|------|
| **다른 장비** (같은 LAN) | `http://192.168.0.92:8106` |
| 이 머신의 **다른 컨테이너** | `http://nginx:80` — `pdf-network`에 붙인다 |
| 이 머신의 **호스트 프로세스** | `http://127.0.0.1:8106` |

`http://nginx:80`은 같은 머신의 도커 네트워크 안에서만 해석되는 이름입니다.
다른 장비에서는 호스트 IP를 쓰세요. 같은 머신의 컨테이너라면 아래처럼 네트워크를 붙일 수 있습니다.
